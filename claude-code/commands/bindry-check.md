...
1. This needs `bindry.config.json` in the project root (written by a previous `/bindry-sync` run) to know which
   Stack, API base, and pinned version (if any) to check against. If it's missing, tell the user to run
...
5. If this project is pinned to a Stack version, the report ends with one of two lines, and they mean different
   things. "pinned to X, which is the newest published version" means there is nothing to do. "pinned to X; the
   Stack has since published Y" is **not** staleness — the project is deliberately not following the Stack.
