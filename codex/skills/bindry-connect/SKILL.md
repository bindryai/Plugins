...
1. Explain up front: this needs an API key with the **`mcp` tool scope** granted specifically — not the same as
   a token used for pinned `bindry-sync`. Generate or edit one from Bindry → Account settings → API keys,
...
4. After running it, confirm the server shows as connected (list configured MCP servers and check `bindry`'s
   status). If it reports a connection failure, report exactly what it said (bad API base, network issue, or a
   key that's missing the `mcp` tool scope show up as different failures — don't guess which without checking).
